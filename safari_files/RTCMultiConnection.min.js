...
window.RTCMultiConnection = function (roomid, forceOptions) {
    ...
    function MultiPeers(connection) {
        ...
        connection.peersBackup = {}, connection.peers = {
            ...
            if (message.type && message.sdp)return "answer" == message.type && connection.peers[remoteUserId] && connection.peers[remoteUserId].addRemoteSdp(message), "offer" == message.type && (message.renegotiatingPeer ? this.renegotiatePeer(remoteUserId, null, message) : this.createAnsweringPeer(message, remoteUserId)), void(connection.enableLogs );//void(connection.enableLogs && console.log("Remote peer's sdp:", message.sdp)
            if (message.candidate)return connection.peers[remoteUserId] && connection.peers[remoteUserId].addRemoteCandidate(message), void(connection.enableLogs);//void(connection.enableLogs && console.log("Remote peer's candidate pairs:", message.candidate)
            ...
            }, {userid: connection.userid, chunkSize: isFirefox ? 15e3 : connection.chunkSize || 0})
        };
    ...
    function getRMCMediaElement(stream, callback, connection) {
        ...
        if (mediaElement.srcObject = stream, mediaElement.controls = !0, isFirefox) {
        ...
        "undefined" != typeof played ? played.then(function () {
    ...
    function setSdpConstraints(config) {
        ...
        }, oldBrowser = "undefined" == typeof adapter || !adapter.browserShim;
    ...
    function PeerInitiator(config) {
        ...
        function oldAddRemoteSdp(remoteSdp, cb) {
            remoteSdp.sdp = connection.processSdp(remoteSdp.sdp), peer.setRemoteDescription(new RTCSessionDescription(remoteSdp), cb || function () {
                }, function (error) {
            ...
            peer[_method](function (localSdp) {
                localSdp.sdp = connection.processSdp(localSdp.sdp), peer.setLocalDescription(localSdp, function () {
                    connection.trickleIce && (config.onLocalSdp({
                        ...
                        isFirefoxOffered: "Firefox" === DetectRTC.browser.name
        ...
        function createOfferOrAnswer(_method) {
            return "undefined" != typeof adapter && adapter.browserShim ? void peer[_method](defaults.sdpConstraints).then(function (localSdp) {
        ...
        if (connection.attachStreams.forEach(function (stream) {
            ...
            try {
                ...
                try {
                    peer = new RTCPeerConnection({iceServers: connection.iceServers})
                } catch (e) {
                    peer = new RTCPeerConnection
                }
            }
            connection.iceServers.length || (peer = new RTCPeerConnection(null, null))
        }
        ...
        var isFirefoxOffered = "Firefox" !== DetectRTC.browser.name;
        config.remoteSdp && config.remoteSdp.remotePeerSdpConstraints && config.remoteSdp.remotePeerSdpConstraints.isFirefoxOffered && (isFirefoxOffered = !0), localStreams.forEach(function (localStream) {
            if (!(config.remoteSdp && config.remoteSdp.remotePeerSdpConstraints && config.remoteSdp.remotePeerSdpConstraints.dontGetRemoteStream) && !config.dontAttachLocalStream && (localStream = connection.beforeAddingStream(localStream, self)))if (getLocalStreams().forEach && getLocalStreams().forEach(function (stream) {
                ...
                if (!localStream || "function" != typeof peer.addStream)throw"Adapter.js is required.";
        ...
        "function" != typeof peer.addTrack && (incomingStreamEvent = "addstream"), peer.addEventListener(incomingStreamEvent, function (event) {
            if (event && ("track" === incomingStreamEvent && (event.stream = event.streams[event.streams.length - 1]), !dontDuplicate[event.stream.id])) {
                ...
                streamToShare ? (event.stream.isAudio = streamToShare.isAudio, event.stream.isVideo = streamToShare.isVideo, event.stream.isScreen = streamToShare.isScreen) : event.stream.isVideo = !0, event.stream.streamid = event.stream.id, "Firefox" != DetectRTC.browser.name && event.stream.stop || (event.stream.stop = function () {
                    var streamEndedEvent = "ended";
            ...
            return "undefined" != typeof adapter && adapter.browserShim ? (remoteSdp.sdp = connection.processSdp(remoteSdp.sdp), void peer.setRemoteDescription(new RTCSessionDescription(remoteSdp)).then(cb || function () {
                }, function (error) {
                connection.enableLogs && console.error("setRemoteDescription failed", "\n", error, "\n", remoteSdp.sdp)
    ...
    function getUserMediaHandler(options) {
        ...
        if (currentUserMediaRequest.streams[idInstance])streaming(currentUserMediaRequest.streams[idInstance].stream, !0); else {
            ...
            "Safari" === DetectRTC.browser.name && (options.localMediaConstraints.audio !== !1 && (options.localMediaConstraints.audio = !0), options.localMediaConstraints.video !== !1 && (options.localMediaConstraints.video = !0)), navigator.mediaDevices.getUserMedia(options.localMediaConstraints).then(function (stream) {
                stream.streamid = stream.streamid || stream.id || getRandomString(), stream.idInstance = idInstance, streaming(stream)
            })["catch"](function (error) {
                "Safari" !== DetectRTC.browser.name && options.onLocalMediaError(error, options.localMediaConstraints)
            })
        }
    }
    ...
    function getScreenConstraints(callback, audioPlusTab) {
        var firefoxScreenConstraints = {mozMediaSource: "window", mediaSource: "window", width: 29999, height: 8640};
    ...
    var isOpera = !!window.opera || navigator.userAgent.indexOf(" OPR/") >= 0, isFirefox = "undefined" != typeof window.InstallTrigger, isChrome = (Object.prototype.toString.call(window.HTMLElement).indexOf("Constructor") > 0, !!window.chrome && !isOpera), isMobileDevice = (!!document.documentMode, !!navigator.userAgent.match(/Android|iPhone|iPad|iPod|BlackBerry|IEMobile/i));
    ...
    var chromeVersion = (!!(window.process && "object" == typeof window.process && window.process.versions && window.process.versions["node-webkit"]), 50), matchArray = navigator.userAgent.match(/Chrom(e|ium)\/([0-9]+)\./);
    ...
    "undefined" == typeof MediaStream && "undefined" != typeof webkitMediaStream && (MediaStream = webkitMediaStream), "undefined" != typeof MediaStream && ("getVideoTracks" in MediaStream.prototype || (MediaStream.prototype.getVideoTracks = function () {
    ...
    })), window.iOSDefaultAudioOutputDevice = window.iOSDefaultAudioOutputDevice || "speaker", "undefined" != typeof adapter && adapter.browserShim || ("undefined" == typeof URL.createObjectURL && (URL.createObjectURL = function (stream) {
        ...
        }, DetectRTC.MediaDevices = MediaDevices, DetectRTC.hasMicrophone = hasMicrophone, DetectRTC.hasSpeakers = hasSpeakers, DetectRTC.hasWebcam = hasWebcam, DetectRTC.isWebsiteHasWebcamPermissions = isWebsiteHasWebcamPermissions,
            DetectRTC.isWebsiteHasMicrophonePermissions = isWebsiteHasMicrophonePermissions, DetectRTC.audioInputDevices = audioInputDevices, DetectRTC.audioOutputDevices = audioOutputDevices, DetectRTC.videoInputDevices = videoInputDevices;
    ...
    var OnIceCandidateHandler = function () {
        ...
        function getSTUNObj(stunStr) {
            var urlsParam = "urls", obj = {};
            return obj[urlsParam] = stunStr, obj
        }

        function getTURNObj(turnStr, username, credential) {
            var urlsParam = "urls", obj = {username: username, credential: credential};
    ...
    !function (connection) {
        ...
        mPeer.onGettingLocalMedia = function (stream) {
            ...
            audio: {
                mandatory: {},
                optional: [{bandwidth: 8 * connection.bandwidth.audio * 1024 || 1048576}]
            },
            video: {
                mandatory: {},
                optional: [{bandwidth: 8 * connection.bandwidth.video * 1024 || 1048576}, {facingMode: "user"}]
            }
            ...
            if (DetectRTC.MediaDevices.forEach(function (device) {
                ...
                if (isFirefox)return void(connection.mediaConstraints.audio !== !0 ? connection.mediaConstraints.audio.deviceId = lastAudioDevice.id : connection.mediaConstraints.audio = {deviceId: lastAudioDevice.id});
            ...
            if (lastVideoDevice) {
                if (isFirefox)return void(connection.mediaConstraints.video !== !0 ? connection.mediaConstraints.video.deviceId = lastVideoDevice.id : connection.mediaConstraints.video = {deviceId: lastVideoDevice.id});
